    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={handleLogout} className="text-gray-400 hover:text-white text-sm font-medium">
                Sign Out
              </button>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">Your Peers</h1>
              <p className="text-gray-400 mt-1">
                {student.major} @ {student.college}
              </p>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold text-blue-400">{endorsementsRemaining}</div>
        ...
        {/* Error State */}
        {error && (
          <div className="bg-white/[0.04] backdrop-blur-md border border-white/[0.08] rounded-2xl p-8 text-center">
        ...
        {!error && peers.length === 0 && (
          <div className="bg-white/[0.04] backdrop-blur-md border border-white/[0.08] rounded-2xl p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-white mb-2">No Peers Yet</h2>
        ...
        {!error && peers.length > 0 && (
          <div className="space-y-4">
            {peers.map(peer => (
              <div key={peer.id} className="bg-white/[0.04] backdrop-blur-md border border-white/[0.08] rounded-2xl p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {peer.skills && peer.skills.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-3">
                        {peer.skills.map((skill, idx) => (
                          <span
                            key={idx}
                            className="px-3 py-1 bg-blue-500/10 text-blue-400 text-sm rounded-full font-medium"
                          >
                            {skill}
                          </span>
                        ))}
                    ...
                    {peer.bio && (
                      <p className="text-gray-400 mt-3 text-sm">{peer.bio}</p>
                    )}
                  ...
                  <div className="ml-4">
                    {peer.hasEndorsed ? (
                      <span className="inline-flex items-center px-4 py-2 bg-green-500/20 text-green-400 rounded-lg font-semibold text-sm">
                      ...
                      <span className="inline-flex items-center px-4 py-2 bg-white/10 text-gray-500 rounded-lg font-semibold text-sm">
      ...
      {showModal && selectedPeer && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
          <div className="bg-white/[0.04] backdrop-blur-md border border-white/[0.08] rounded-2xl max-w-md w-full p-6">
            ...
            {endorseMessage.text && (
              <div className={`p-3 rounded-lg mb-4 ${
                endorseMessage.type === 'success' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
              }`}>
                {endorseMessage.text}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={endorsing}
                className="flex-1 px-4 py-3 bg-white/10 text-gray-300 rounded-lg font-semibold hover:bg-white/10 transition-all disabled:opacity-50"
              >
                Cancel
              </button>
              <button
